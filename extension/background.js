...
window.youtube = Youtube; //debugging
window.immutable = Immutable; //debugging
window.reload = chrome.runtime.reload;
let extensionTabs = immutable_1.OrderedSet();
...
chrome.browserAction.onClicked.addListener((tab) => {
    if (!extensionTabs.size && !isCreatingTab) {
        ...
        chrome.tabs.get(extensionTabs.first(), (tab) => {
...
},{"./youtube":6,"immutable":1}],3:[function(require,module,exports){
"use strict";
...
const User = immutable_1.Record(new UserClass(), "User");
//const User = Record({
...
class SubscriptionClass {
    constructor() {
        this.watched = immutable_1.List();
        this.unwatchedCount = 0;
    }
}
const Subscription = immutable_1.Record(new SubscriptionClass(), "Subscription");
//const Subscription = Record({
//	watched: List<string>(),
//	unwatchedCount: 0
//}) as any as RecordClass<Subscription>;
//interface Subscription extends Map<string, any> {
//	watched: List<string>,
//	unwatchedCount: number
//}
//interface ISubscriptionRecord extends Map<string, any> {
//	new (values?: {[key: string]: any}): this;
//	(values?: {[key: string]: any}): this;
//	watched: List<string>,
//	unwatchedCount: number
//}
//
...
function init() {
    return Storage.get("users").then((value) => {
        if (value === undefined) {
            exports.users = immutable_1.Map();
        }
        else if (typeof value === "object") {
            exports.users = parseObj(value);
            console.log("users");
            console.log(exports.users);
        }
        else
            throw new errors_1.StorageError("users is the wrong type: " + typeof value + ": " + value);
    });
}
exports.init = init;
function parseObj(usersObj) {
    let users = immutable_1.Map(usersObj);
    return users.map((userObj) => {
        let user = User(userObj);
        return user.set("subscriptions", immutable_1.Map(user.subscriptions).map((subscriptionObj) => {
            let subscription = Subscription(subscriptionObj);
            return subscription.set("watched", immutable_1.List(subscription.watched));
        }));
    });
}
function setUser(id) {
    if (!exports.users.has(id)) {
        exports.users = exports.users.set(id, User());
    }
    currentUserId = id;
}
exports.setUser = setUser;
function setSubscriptions(channelIds) {
    //TODO filter + handle deleted subs
    exports.users = exports.users.setIn([currentUserId, "subscriptions"], exports.users.get(currentUserId).subscriptions.withMutations((subs) => {
...
function getSubscriptions() {
    return exports.users.get(currentUserId).subscriptions.keySeq();
}
exports.getSubscriptions = getSubscriptions;
function setWatched(channelId, videoId) {
    if (!isWatched(channelId, videoId)) {
        let channel = exports.users.get(currentUserId).subscriptions.get(channelId);
        let watched = channel.watched.push(videoId).take(100);
        exports.users = exports.users.setIn([currentUserId, "subscriptions", channelId], channel.set("watched", watched)
...
function setUnwatched(channelId, videoId) {
    let channel = exports.users.get(currentUserId).subscriptions.get(channelId);
    let watched = channel.watched;
    if (watched.includes(videoId)) {
        exports.users = exports.users.setIn([currentUserId, "subscriptions", channelId], channel.set("watched", watched.filterNot((id) => id === videoId))
            .set("unwatchedCount", channel.get("unwatchedCount") + 1));
        return save();
    }
    else
...
exports.setUnwatched = setUnwatched;
function isWatched(channelId, videoId) {
    let watched = getWatchedVideos(channelId);
    return watched.includes(videoId);
}
exports.isWatched = isWatched;
function getWatchedVideos(channelId) {
    return exports.users.get(currentUserId).subscriptions.get(channelId).watched;
}
exports.getWatchedVideos = getWatchedVideos;
function updateWatchedCount(channelId, uploadIds) {
    let watched = getWatchedVideos(channelId);
    let count = uploadIds.count((id) => !watched.includes(id));
    exports.users = exports.users.setIn([currentUserId, "subscriptions", channelId, "unwatchedCount"], count);
    return save();
}
exports.updateWatchedCount = updateWatchedCount;
function getUnwatchedCount(channelId) {
    return exports.users.get(currentUserId).subscriptions.get(channelId).unwatchedCount;
}
exports.getUnwatchedCount = getUnwatchedCount;
function save() {
    return Storage.set("users", exports.users.toJS());
}
},{"./errors":3,"./storage":4,"immutable":1}],6:[function(require,module,exports){
/**
...
const errors_1 = require("./errors");
const immutable_1 = require("immutable");
var youtubeApi_2 = require("./youtubeApi");
exports.isAuthorized = youtubeApi_2.isAuthorized;
var user_1 = require("./user");
exports.getSubscriptions = user_1.getSubscriptions;
exports.isWatched = user_1.isWatched;
exports.setWatched = user_1.setWatched;
exports.setUnwatched = user_1.setUnwatched;
exports.getWatchedVideos = user_1.getWatchedVideos;
exports.getUnwatchedCount = user_1.getUnwatchedCount;
let MAX_VIDEOS_PER_CHANNEL = 50;
class ChannelClass {
    constructor() {
        this.uploads = immutable_1.List();
        this.uploadsPlaylist = "";
        this.name = "";
        this.thumb = "";
    }
}
const Channel = immutable_1.Record(new ChannelClass(), "Channel");
exports.user = User;
function init() {
    return Promise.all([
        User.init(),
        ...
        Storage.get("channels").then((value) => {
            if (value === undefined) {
                exports.channels = immutable_1.Map();
            }
            else if (typeof value === "object") {
                exports.channels = parseObj(value);
                console.log("channels");
                console.log(exports.channels);
            }
            else
                throw new errors_1.StorageError("channels is the wrong type: " + typeof value + ": " + value);
        })
    ]).then(([, authorized]) => authorized);
}
exports.init = init;
function parseObj(channelsObj) {
    let channels = immutable_1.Map(channelsObj);
    return channels.map((channelObj) => {
        let channel = Channel(channelObj);
        return channel.set("uploads", immutable_1.List(channel.uploads).map((videoObj) => youtubeApi_1.Video(videoObj)));
    });
}
function authorize() {
...
function fetchSubscriptions() {
    return YoutubeApi.fetchSubscriptions().then((subs) => {
        let getChannelUploadsPlaylistPromises = [];
        let channelsMutable = exports.channels.asMutable();
        subs.forEach((sub, id) => {
            if (!channelsMutable.has(id)) {
                channelsMutable.set(id, Channel(sub));
                getChannelUploadsPlaylistPromises.push(YoutubeApi.fetchChannelUploadsPlaylist(id).then((playlistId) => {
        ...
        return Promise.all(getChannelUploadsPlaylistPromises).then(() => {
            exports.channels = channelsMutable.asImmutable();
            return subs.keySeq();
        });
    });
}
function updateSubscriptionsUploads() {
    return updateChannelsUploads(User.getSubscriptions());
}
exports.updateSubscriptionsUploads = updateSubscriptionsUploads;
function updateChannelsUploads(channelIds) {
    let promises = channelIds.map((id) => {
        return fetchChannelUploads(id).then((uploads) => {
            if (uploads)
                User.updateWatchedCount(id, uploads.map((video) => video.id)); //immutable types please
        });
    });
    return Promise.all(promises.toJS()).then(() => {
        Storage.set("channels", exports.channels.toJS());
    });
}
function fetchChannelUploads(channelId) {
    let channel = exports.channels.get(channelId);
    return YoutubeApi.fetchPlaylistItems(channel.uploadsPlaylist).then((videoIds) => {
        ...
        if (newVideoIds.size) {
            return fetchVideosData(newVideoIds).then((videos) => {
                uploads = videos.sort(sortVideosByUploadTime).concat(uploads).take(MAX_VIDEOS_PER_CHANNEL);
                exports.channels = exports.channels.setIn([channelId, "uploads"], uploads);
                return uploads;
            });
        }
        else
...
function fetchVideosData(videoIds) {
    ...
    if (videoIdsString != "") {
        ...
        return Promise.resolve(immutable_1.List());
    }
}
function getChannelName(channelId) {
    checkChannelLoaded(channelId);
    return exports.channels.get(channelId).name;
}
exports.getChannelName = getChannelName;
function getChannelThumb(channelId) {
    checkChannelLoaded(channelId);
    return exports.channels.get(channelId).thumb;
}
exports.getChannelThumb = getChannelThumb;
function getChannelUploads(channelId) {
    checkChannelLoaded(channelId);
    return exports.channels.getIn([channelId, "uploads"]);
}
exports.getChannelUploads = getChannelUploads;
function checkChannelLoaded(channelId) {
    if (!exports.channels.has(channelId))
        throw new errors_1.IllegalArgumentError(channelId + " is not a loaded channel");
}
},{"./errors":3,"./storage":4,"./user":5,"./youtubeApi":7,"immutable":1}],7:[function(require,module,exports){
/**
...
exports.Video = immutable_1.Record(new VideoClass(), "Video");
...
let accessTokenExpireTime = null; // todo actually update this
...
function getAuthorizationToken() {
    if (!exports.isAuthorized)
        return Promise.reject(new errors_1.AuthorizationError("Not authorized"));
...
function authorize() {
    if (exports.isAuthorized)
        return Promise.reject(new errors_1.AuthorizationError("Already logged in"));
    return launchWebAuthFlow(AUTH_URL /* + "&prompt=select_account"*/, true)
        .then(handleAuthorizationResult)
        .then((accessToken) => {
        if (!accessToken)
            throw new errors_1.AuthorizationError("No access token?");
        return true;
    }).catch((error) => {
        if (!(error instanceof errors_1.AuthorizationError) || error.message != "The user did not approve access.")
            throw error;
        else
            return false;
    });
...
function handleAuthorizationResult(result) {
    ...
    exports.isAuthorized = accessToken != null;
    isAccessTokenValid = exports.isAuthorized;
    accessTokenExpireTime = exports.isAuthorized ? new Date(Date.now() + Number(result.get("expires_in")) * 1000) : null;
    if (exports.isAuthorized) {
        return accessToken;
    }
    ...
    return new Promise((resolve, reject) => {
        chrome.identity.launchWebAuthFlow({
            ...
            if (chrome.runtime.lastError) {
                ...
                reject(new errors_1.AuthorizationError(chrome.runtime.lastError.message));
            }
            else {
                ...
                if (responseUrl) {
                    ...
                    reject(new errors_1.AuthorizationError("No response URL?"));
            }
        });
    });
}
function fetchUserId() {
    return exports.channels.get({
        mine: true,
        part: "id",
        fields: "items/id"
...
function fetchSubscriptions() {
    return exports.subscriptions.get({
        mine: true,
        part: "id,snippet",
        fields: "nextPageToken,items(snippet(title,resourceId/channelId,thumbnails))",
        maxResults: 50,
        order: "alphabetical"
    }, true).then(fetchSubscriptionsNextPage).then((items) => immutable_1.OrderedMap(immutable_1.List(items).map((subscriptionItem) => [subscriptionItem.snippet.resourceId.channelId, {
...
function fetchSubscriptionsNextPage(response) {
...
exports.fetchChannelUploadsPlaylist = fetchChannelUploadsPlaylist;
function fetchPlaylistItems(playlistId) {
    return exports.playlistItems.get({
        playlistId: playlistId,
        part: "contentDetails",
        fields: "items/contentDetails/videoId",
        maxResults: 50
    }).then((response) => {
        let items = immutable_1.List(response.items);
        return items.map((video) => video.contentDetails.videoId);
    });
}
exports.fetchPlaylistItems = fetchPlaylistItems;
function fetchVideoDetails(videoIds) {
    return exports.videos.get({
        id: videoIds,
...
class YoutubeResource {
    ...
    get(options, authorized) {
        return YoutubeResource.ajax(this.resource, "GET", options, authorized);
    }
    post(options, authorized) {
        return YoutubeResource.ajax(this.resource, "POST", options, authorized);
    }
    put(options, authorized) {
        return YoutubeResource.ajax(this.resource, "PUT", options, authorized);
    }
    del(options, authorized) {
        return YoutubeResource.ajax(this.resource, "DELETE", options, authorized);
    }
    static ajax(resource, method, params, authorized) {
        let promise;
        ...
        return promise.then(() => {
            return YoutubeResource.fetchOK(url, options);
        }).catch((error) => {
    ...
    static fetchOK(url, options) {
        return fetch(url, options).then((response) => {
            if (response.ok)
                return response.json();
            else {
                console.error(response.json());
                throw new errors_1.XHRError(response.status, response.statusText);
            }
        });
    }
}
YoutubeResource.BASE_URL = "https://www.googleapis.com/youtube/v3/";
YoutubeResource.API_KEY = "AIzaSyClOj2RmQTkYbfqL4o8mBhzx8Jwo-mNhpo";
...
},{"./errors":3,"immutable":1}]},{},[2])
//# sourceMappingURL=data:application/json;charset=utf-8;base64,
